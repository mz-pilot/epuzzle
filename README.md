...
#### Windows (Visual Studio 2022/2025)
1. **Установите**:
...
**Почему мы используем `GTEST_SKIP()`:** Для временного отключения тестов для еще не реализованного функционала (например, `Reasoning` solver) без их удаления из кодовой базы.
...
1. **Strategy** (через `ISolver`) — алгоритмы решения
2. **Factory Method** (`ISolver::create`) — создание объектов
3. **Dependency Injection** — конфигурация и данные
4. **Fan-out/Fan-in** — параллельная обработка
5. **Type Safety** — сильные типы для предотвращения ошибок
6. **RAII** — управление ресурсами (потоки, мьютексы)
7. **Iterator** — обход пространства кандидатов
8. **Variant-based Visitor** — обработка различных типов constraints
